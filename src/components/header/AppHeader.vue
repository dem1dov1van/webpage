      ...
      <nav class="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8" aria-label="Global">
        <div class="flex lg:flex-1">
          <nuxt-link to="/store/products/" class="-m-1.5 p-1.5">
            <span class="sr-only">Selectel Furniture Sale</span>
            <s-logo class="max-w-[50px]"></s-logo>
          </nuxt-link>
        </div>
...
<!--          </Popover>-->
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
          <header-account-info></header-account-info>
        </div>
      </nav>
      <Dialog class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
        <div class="fixed inset-0 z-10 bg-black/[.7]" />
        <DialogPanel class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-4 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div class="flex items-center justify-between">
            <a href="#" class="-m-1.5 p-1.5">
              <span class="sr-only">Selectel Furniture Sale</span>
              <s-logo class="max-w-[50px]"></s-logo>
            </a>
            <button type="button" class="-m-2.5 rounded-md p-2.5 pr-1 text-gray-700" @click="mobileMenuOpen = false">
